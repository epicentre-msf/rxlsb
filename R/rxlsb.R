...
#'
#' @export
rxlsb <- function(path, sheet = 1, col_names = TRUE, col_types = NULL, na = "") {
  wb <- .workbook(path)
...
.resolve_col_types <- function(col_types, n_col) {
  ...
  if (length(bad)) {
    stop(
      "`col_types` must be one of: ", paste(.col_type_values, collapse = ", "),
  ...
  if (length(col_types) != n_col) {
    stop(
      "`col_types` must have length 1 or ", n_col, " (one per column), not ",
      length(col_types), ".",
      call. = FALSE
    )
  }
...
.resolve_sheet <- function(wb, sheet) {
  ...
  if (is.character(sheet)) {
    if (length(sheet) != 1L || !sheet %in% names) {
      stop(
        "Sheet '", sheet, "' not found. Available sheets: ",
  ...
  if (is.numeric(sheet) && length(sheet) == 1L) {
    if (sheet < 1 || sheet > length(names)) {
      stop(
        "Sheet index ", sheet, " is out of range (workbook has ",
        length(names), " sheet(s)).",
        call. = FALSE
      )
    }
    return(names[[as.integer(sheet)]])
  }
  stop("`sheet` must be a single sheet name or positive integer.", call. = FALSE)
}
